...
This site is built by an **orchestrator + workhorse agent system**. A **Fable 5** main session acts as engineering lead — it plans, briefs, adjudicates review findings, and owns quality; it never bulk-produces artifacts. Production work runs through tiered subagents: code and the adversarial review on **Opus 4.8**; tests, quality gates, environment work, and screenshots on **Sonnet 5** (#81). Every change passes an **independent code review** and a **quality gate** that exercises acceptance criteria for real, plus **screenshot verification** for anything user-visible. PRs **self-merge** once those gates pass. **Tural reviews by using the product on staging** — never by reading code — and his feedback becomes new issues. His explicit word is required only for decisions that are his: production cutover, spend, deletions outside this repo, outward-facing acts, and brand calls not settled in the PRD.
